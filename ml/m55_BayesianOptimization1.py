param_bounds = {'x1':(-1,5),
                'x2':(0,4),
                }
def y_function(x1,x2):
    return -x1 **2 -(x2 - 2) **2 + 10

# pip install bayesian-optimization
from bayes_opt import BayesianOptimization

optimizer = BayesianOptimization(f=y_function,          # f에는 함수를 넣고
                                 pbounds=param_bounds,  # 그함수에 파라미터를 딕셔너리 형태로 넣는다. 
                                 random_state=1234)
optimizer.maximize(init_points=2,       # 초기값
                   n_iter=20,           # 돌릴횟수
                   )

# |   iter    |  target   |    x1     |    x2     |
# -------------------------------------------------
# |  1        |  9.739    |  0.1491   |  2.488    |
# |  2        |  6.052    |  1.626    |  3.141    |
# |  3        |  6.528    |  1.775    |  1.435    |
# |  4        |  9.741    |  0.1599   |  2.483    |    갱신
# |  5        |  9.445    | -0.4807   |  1.431    |
# |  6        |  5.0      | -1.0      |  0.0      |
# |  7        |  5.0      | -1.0      |  4.0      |
# |  8        |  8.979    | -1.0      |  2.144    |
# |  9        | -19.0     |  5.0      |  0.0      |
# |  10       | -19.0     |  5.0      |  4.0      |
# |  11       |  5.355    |  0.8034   |  0.0      |
# |  12       |  9.63     |  0.5211   |  1.686    |
# |  13       |  9.98     | -0.1395   |  1.992    |    갱신 
# |  14       |  9.923    |  0.2629   |  2.086    |
# |  15       |  9.966    |  0.02993  |  1.817    |
# |  16       |  9.996    |  0.01324  |  2.062    |    갱신
# |  17       |  10.0     |  0.01454  |  1.991    |    갱신
# |  18       |  10.0     |  0.007833 |  1.99     |    갱신
# |  19       |  10.0     |  0.002898 |  1.987    |
# |  20       |  9.996    | -0.01311  |  2.064    |
# |  21       |  9.998    | -0.02176  |  1.963    |
# |  22       |  9.999    |  0.01782  |  1.985    |
# =================================================

print(optimizer.max)
# {'target': 9.999835918969607, 'params': {'x1': 0.00783279093916099, 'x2': 1.9898644972252864}}
