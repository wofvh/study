...
model.add(LSTM(350, input_shape=(4,1)))      # [batch, timesteps(몇개씩 자르는지), feature=1(input_dim)]
...
start = time.time()
model.fit(x_train,y_train, epochs=300, batch_size=128,validation_split=0.2,callbacks=[es,reduced_lr])
end = time.time()-start
