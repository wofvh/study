...
x_train, x_test, y_train, y_test = train_test_split(x_data, y_data,
                                                    train_size=0.7, shuffle=True, random_state=123,stratify=y_data)
...
w = tf.compat.v1.Variable(tf.random_normal([x_data.shape[1],y_data.shape[1]]))
b = tf.compat.v1.Variable(tf.zeros([1,y_data.shape[1]]))
...
#output layer
w4 = tf.compat.v1.Variable(tf.random_normal([20, 3]))
b4= tf.compat.v1.Variable(tf.zeros([3]))
