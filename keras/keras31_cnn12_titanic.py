...
x_train = x_train.reshape(668, 7,1,1)
x_test = x_test.reshape(223, 7,1,1)
...
model.add(Conv2D(64, kernel_size=(1,1),                                         # 자르는 사이즈 (행,렬 규격.) 10= 다음레이어에 주는 데이터
                 padding='same',
                 input_shape=(7,1,1), activation= 'relu'))    #(batch_size, row, column, channels)       # N(장수) 이미지 5,5 짜리 1 흑백 3 칼라                                                                        
...
model.fit(x_train, y_train, epochs=500, batch_size=34, verbose=1, 
